...
# Works
#-----------------------------------
class Work(ModelDocument):

                           
    class Meta(ModelTree.Meta):
        verbose_name = _('Trabajo')
        verbose_name_plural = _('1. Trabajos')
        unique_together= (('doctype', 'alias'),)


class WorkAux(ModelAuxBase):
    trunk = models.ForeignKey(Work, on_delete=models.CASCADE, 
                                    related_name = "datax_set",
                                    null=True, blank=True)
    class Meta(ModelAuxBase.Meta):
        verbose_name = _('Detalle del Trabajo')
        verbose_name_plural = _('2. Detalle de Trabajos')
